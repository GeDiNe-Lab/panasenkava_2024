...
library(data.table)
percents <- top500$gene %>%
    lapply(function(gene) {
        if (gene %in% rownames(seurat_week4@assays$RNA$counts)) {
            percent <- unique(sc_meta_week4$celltype_region) %>% sapply(function(ct) {
                test <- seurat_week4@assays$RNA$counts[gene, ]
                test <- ifelse(test >= 1, sc_meta_week4$celltype_region, "none")
                test2 <- table(test)
                ncell <- test2[ct]
                if (is.na(ncell)) {
                    ncell <- 0
                }
                return(unname(ncell / nrow(filter(sc_meta_week4, celltype_region == ct)) * 100))
            })
        } else {
            percent <- rep(0, 8)
        }
    ...
    do.call(rbind, .) %>%
    as.data.frame()
rownames(percents) <- top500$gene
View(percents)
percents_tot <- percents
percents_tot$total <- rowSums(percents) / 8
View(percents_tot)

write.csv(percents_tot, file = "Figure_5/SHH_cluster_500_percents.csv", quote = FALSE)
